...
export class DrawCpClausesComponent implements OnInit {
    ...
    current_time = moment().format("mm:ss");
    ...
    */
    ...
    ngOnInit() {
        ...
        this.current_time = moment().format("mm:ss");
    ...
    changeTimezone(event) {
        this.usaTime = new Date().toLocaleString("en-US", { timeZone: event.value });
        this.usaTime = new Date(this.usaTime);
        console.log('timeZone: ' + this.usaTime.toLocaleString())
        this.SelectedTimeZone = event.value;
        console.log(event.value);
        // this.usaTime.split(',');

        this.utc = this.usaTime.toLocaleString().split(',');
        console.log(this.usaTime.getTime());
    ...
    changeVessel() {
        ...
        for (let index = 0; index < this.vesselRecordsServerSideResponseData.length; index++) {
            if (this.vesselId == this.vesselRecordsServerSideResponseData[index].id) {
                this.lifted_vessel_name = this.vesselRecordsServerSideResponseData[index].username;
            }
        }
        var updateData = {};
        updateData['ownerId'] = this.ownerId;
    ...
    stdBidSubmit() {
        ...
        if (this.lifted_vessel_name == null && this.lifted_vessel_imo == null && this.lifted_vessel_flag == null && this.lifted_vessel_year_built == null) {
            this.fetchVesselData();
            ...
            try {
                this._userService.TradingData(drawCondition).pipe(first()).subscribe((res) => {
                    ...
                    if (this.tradingResponse.success === true) {
                        ...
                        var current_time = (cpTime != '' && cpTime != null) ? cpTime : moment(new Date()).format("mm:ss");
